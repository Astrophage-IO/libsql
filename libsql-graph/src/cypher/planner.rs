...
fn plan_match(m: &MatchStatement) -> Result<QueryPlan, String> {
    ...
    while let Some(elem) = iter.next() {
        if let PatternElement::Relationship(rel) = elem {
            let to_node = iter
                .next()
                .ok_or("relationship must be followed by a node")?;
...
mod tests {
    ...
    fn test_plan_simple_match() {
        ...
        assert!(
            matches!(&plan.steps[0], PlanStep::NodeScan { label: Some(l), .. } if l == "Person")
        );
    ...
    fn test_plan_match_with_relationship() {
        ...
        assert!(
            matches!(&plan.steps[1], PlanStep::Expand { rel_type: Some(t), .. } if t == "KNOWS")
        );
    ...
    fn test_plan_match_with_set() {
        ...
        assert!(plan
            .steps
            .iter()
            .any(|s| matches!(s, PlanStep::SetProperty { property, .. } if property == "age")));
    }
    ...
    fn test_plan_create_node() {
        ...
        assert!(
            matches!(&plan.steps[0], PlanStep::CreateNode { label: Some(l), .. } if l == "Person")
        );
    }
    ...
    fn test_plan_detach_delete() {
        ...
        assert!(plan
            .steps
            .iter()
            .any(|s| matches!(s, PlanStep::DeleteNode { detach: true, .. })));
    }
    ...
    fn test_plan_order_limit() {
        ...
        assert!(plan
            .steps
            .iter()
            .any(|s| matches!(s, PlanStep::OrderBy { .. })));
        assert!(plan
            .steps
            .iter()
            .any(|s| matches!(s, PlanStep::Limit { count: 10 })));
    }
